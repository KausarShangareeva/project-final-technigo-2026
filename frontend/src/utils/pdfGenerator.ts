...
export function generateSchedulePDF(items: ScheduleItem[], userName: string = "User") {
  ...
  doc.setFontSize(8);
  doc.setTextColor(COLORS.textMuted);
  doc.text("Generated by PlanFlow - Your Islamic Learning Companion", pageWidth / 2, footerY, {
    align: "center",
  });

  // === DOWNLOAD ===
  const fileName = `PlanFlow_Schedule_${new Date().toISOString().split("T")[0]}.pdf`;
  doc.save(fileName);
}
